...
macro_rules! eprint {
    ($($arg:tt)*) => {{
        $crate::serial_eprint!($($arg)*);
        $crate::vga_buffer::_eprint(format_args!($($arg)*));
        $crate::vga_buffer::flush(); $crate::serial::flush();
    }};
}
...
pub fn _print(args: fmt::Arguments) {
    if !INITIALISED.load(Ordering::SeqCst) {
        if let Some(mut writer) = WRITER.try_lock() {
            writer.color_code = DEBUG_COLOUR;
    ...
    BUFFER.read().push((result, DEBUG_COLOUR));
}
...
pub fn flush() {
    if let Some(mut s) = WRITER.try_lock() {
        ...
        while !old.is_empty() || Arc::strong_count(&old) != 1 {
            while let Ok((line, colour)) = old.pop() {
                s.color_code = colour;
                s.write_string(&line);
            }
        }
